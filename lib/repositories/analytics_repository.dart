...
class AnalyticsRepository {
  ...
  Future<Map<String, dynamic>> getPlatformStats() async {
    try {
      ...
      final eventsRes = await _client.from('events').select('id, status, gross_revenue, total_sold');
      final bookingsRes = await _client.from('bookings').select('id, total_amount, status, created_at');
      ...
      final totalAttendees = usersList.where((u) => u['role'] == 'attendee').length;
      final totalOrganizers = usersList.where((u) => u['role'] == 'organizer').length;

      final totalEvents = eventsList.length;
      final publishedEvents = eventsList.where((e) => e['status'] == 'published').length;
      final pendingEvents = eventsList.where((e) => e['status'] != 'published').length;
